...
int main(int argc, char const* argv[]){
    ...
    if (argc < 3){
	printf("./DistributionValueParallel.out <matrix Size> <P must be >= 2 or <= 64 >\n");
	exit(-1);
     }
  ...
  /*Assigning Row Boundary Value to each strips*/ 
   ...
   /*Spanning Threads*/
   for (index = 0; index < numberOfThread; ++index){
        create_error = pthread_create(&threads[index], NULL, calculateDistributionCount, (void *)&stripInfo[index]);
        if(create_error){
