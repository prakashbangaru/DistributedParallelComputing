...
/*assignMatrix(destinantion,source)*/
void assignMatrix(Matrix *A, Matrix B){
        
        A->rowStart = B.rowStart;
        A->rowEnd = B.rowEnd;
        A->columnStart = B.columnStart;
        A->columnEnd = B.columnEnd;
        A->matrixValue = B.matrixValue;
}

void getMatrixAdditionAttribute(MatrixAddAttr *add1,Matrix A11, Matrix B11, Matrix C11, Matrix A12,Matrix B21,Matrix D11,int size){
            
            assignMatrix(&(add1->A1), A11);
            assignMatrix(&(add1->B1), B11);
            assignMatrix(&(add1->C1), C11);
            assignMatrix(&(add1->A2), A12);
            assignMatrix(&(add1->B2), B21);
            assignMatrix(&(add1->D2), D11);
            assignMatrix(&(add1->result),C11);
            add1->size = size;
}

Matrix  subsetMatrixMultiplication(Matrix A, Matrix B, Matrix C, int size){
    ...
    // Variables used for Strassen's Matrix Multiplication
     ...
     if( size <= MIN_MATRIX_SIZE ){
        ...
        C.matrixValue[C.rowEnd][C.columnEnd]= P+R-Q+U;
         ...
         matrixAddition(subsetMatrixMultiplication(A11,B11,C11,halfSize),subsetMatrixMultiplication(A12,B21,D11,halfSize),C11);
         matrixAddition(subsetMatrixMultiplication(A11,B12,C12,halfSize),subsetMatrixMultiplication(A12,B22,D12,halfSize),C12);
         matrixAddition(subsetMatrixMultiplication(A21,B11,C21,halfSize),subsetMatrixMultiplication(A22,B21,D21,halfSize),C21);
         matrixAddition(subsetMatrixMultiplication(A21,B12,C22,halfSize),subsetMatrixMultiplication(A22,B22,D22,halfSize),C22);
...
void thread_run(void *arg){
         ...
         MatrixAddAttr *add = (MatrixAddAttr*)arg;
 
         matrixAddition(subsetMatrixMultiplication(add->A1,add->B1,add->C1,add->size),subsetMatrixMultiplication(add->A2,add->B2,add->D2,add->size),add->C1);
}
        ...
        C.matrixValue[C.rowEnd][C.columnEnd]= P+R-Q+U;
     	...
     	//	Below Memset OPeration is a costly Operation
       	//	memset(D.matrixValue[index],0,size*sizeof(double));
               ...
               getMatrixAdditionAttribute(&add0,A11,B11,C11,A12,B21,D11,halfSize);
               getMatrixAdditionAttribute(&add1,A11,B12,C12,A12,B22,D12,halfSize);
               getMatrixAdditionAttribute(&add2,A21,B11,C21,A22,B21,D21,halfSize);
               getMatrixAdditionAttribute(&add3,A21,B12,C22,A22,B22,D22,halfSize);
        

         /*Spawning Threads*/
         create_error = pthread_create(&threads[0], NULL, thread_run, (void *)&add0);
         if(create_error){
                printf("pthread_create 0 returned error: %d\n", create_error);
                exit(-1);
         }

         create_error = pthread_create(&threads[1], NULL, thread_run, (void *)&add1);
         if(create_error){
                printf("pthread_create 1 returned error: %d\n", create_error);
                pthread_exit(NULL);
         }
 
         create_error = pthread_create(&threads[2], NULL, thread_run, (void *)&add2);
         if(create_error){
                printf("pthread_create 1 returned error: %d\n", create_error);
                pthread_exit(NULL);
         }
         
         create_error = pthread_create(&threads[3], NULL, thread_run, (void *)&add3);
         if(create_error){
                printf("pthread_create 1 returned error: %d\n", create_error);
                pthread_exit(NULL);
         }
